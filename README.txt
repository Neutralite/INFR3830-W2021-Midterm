If the projects open up with an Untitled scene loaded instead of SampleScene, well, SampleScene's
where the working stuff is.

Controls:
- Press the WASD Keys for movement.
- Click on the chat box input field to use it.
- Press the Return Key to send a chat message.
- Click outside of the chat box input field to return to play.

Updates:
- Current version of client can send things to exercise 3's UDP Server if that's set to listen on port 11112.
- Current version of server is unimplemented.

Bugs:
- If you don't click outside of the chat box input field when done, sometimes it will be activated through any button press. To fix, click outside of the chat box input field.


Have a nice day.